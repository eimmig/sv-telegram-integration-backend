...
def test_follow_up_answer_fills_the_awaited_field_directly(redis_client: redis.Redis) -> None:
    handle_message(redis_client, "user-c", "pt-BR", "odd 1.85")

    result = handle_message(redis_client, "user-c", "pt-BR", "50")

    assert result.status == "pending"
    assert result.message == "Em que data você fez essa aposta? (dd/mm/aaaa)"
    pending = get_pending(redis_client, "user-c")
    assert pending is not None
    assert pending.fields["odd"] == "1.85"
    assert pending.fields["stake"] == "50"
...
def test_completes_and_clears_state_once_every_required_field_is_known(redis_client: redis.Redis) -> None:
    ...
    handle_message(redis_client, "user-d", "pt-BR", "50")

    result = handle_message(redis_client, "user-d", "pt-BR", "08/09/2026")
    ...
    assert result.bet["bet_date"] == "2026-09-08"
...
def test_extracts_everything_from_a_single_complete_message(redis_client: redis.Redis) -> None:
    result = handle_message(
        redis_client, "user-e", "pt-BR", "Bet365, odd 1.85, valor R$ 50,00, em 08/09/2026"
    )
